...
const VideoCard: React.FC<VideoCardProps> = ({ video, onDownload }) => {
  ...
  // Fallback: just get the regular video URL for preview (plays from start)
  const getSimplePreviewUrl = useCallback((publicId: string) => {
    return getCldVideoUrl({
      src: publicId,
      width: 400,
      height: 225,
  ...
  const handlePreviewError = () => {
    // If the fancy preview fails, try the simple one first
  ...
  const currentPreviewUrl = useFallbackPreview
    ? getSimplePreviewUrl(video.publicId)
